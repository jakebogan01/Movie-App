...
//grab base image and search movies by genre
let getConfig = () => {
    ...
    fetch(url)
        ...
        .then((data) => {
            ...
            getMovieByGenre('.popular-movies', true, 35, '27%2C10751%2C12%2C878' );
            getMovieByGenre('.comedy-movies', false, 35, '27%2C10751%2C12%2C878');
            getMovieByGenre('.science-fiction-movies', false, 878, '35%2C12%2C27%2C10751');
            getMovieByGenre('.family-movies', false, 10751, '35%2C12%2C27%2C878');
            getMovieByGenre('.adventure-movies', false, 12, '35%2C10751%2C27%2C878');
            getMovieByGenre('.horror-movies', false, 27, '35%2C10751%2C12%2C878');
        })
        .catch(function (err) {
            alert(err);
        });
...
//searches movie keyword and removes previous search if currently exists
...
//removes search results when input is focused
...
let getFeaturedMovie = (id) => {
    ...
    fetch(url)
        .then(result => result.json())
        .then((data) => {
            const {vote_average, release_date, runtime, homepage, overview} = data;
            let div = document.createElement("DIV");
            let newDate = release_date.slice(0, 4);
            div.innerHTML =
                ...
                <p class="font-semibold my-2 xl:my-5 sm:text-lg lg:text-2xl 2xl:text-3xl max-w-[50.5625rem] xl:max-w-[54.5625rem] transition-all duration-300" style="line-height: 1.7;">${overview}</p>
                <a href="${homepage}" class="inline-flex items-center mt-5 md:mt-8 transition-all duration-300" target="_blank">
...
let showDetails = (id) => {
    ...
    fetch(url)
        .then(result => result.json())
        .then((data) => {
            const {title, poster_path, vote_average, homepage, overview} = data;
            let div = document.createElement("DIV");
                                    ...
                                    <div class="relative flex w-full items-center overflow-hidden bg-[#30385A] rounded-md px-4 pt-14 pb-8 shadow-2xl sm:px-6 sm:pt-8 md:p-6 lg:p-0">
                                        ...
                                        <div class="sm:block lg:grid w-full grid-cols-1 items-start gap-y-8 gap-x-6 sm:grid-cols-12 lg:gap-x-8 overflow-hidden">
                                            <div class="sm:hidden lg:block aspect-w-2 aspect-h-3 overflow-hidden rounded-lg lg:rounded-none bg-[#30385A] sm:col-span-4 lg:col-span-5">
                                                <img src="${'https://image.tmdb.org/t/p/w500/' + poster_path}" alt="${title}" class="object-cover object-center">
                                            </div>
                                            <div class="sm:col-span-8 lg:col-span-7 py-8 pr-8 overflow-y-auto">
                                                <h2 class="text-3xl font-medium text-white sm:pr-12">${title}</h2>
                                                ...
                                                <section aria-labelledby="movie-overview" class="mt-8">
                                                    ...
                                                    <a href="${homepage}" target="_blank" class="mt-8 flex w-full items-center justify-center rounded-md border border-transparent bg-indigo-600 py-3 px-8 text-base font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-all">
                                                        View Details
                                                    </a>
                                                </section>
                                            </div>
...
//grab list of movies by genre
let getMovieByGenre = (elClass, path, genre, withoutGenre) => {
    let url;
    if(path) {
        url = ''.concat(baseURL, 'movie/popular?api_key=', APIKEY, '&language=en-US');
    } else {
        url = ''.concat(baseURL, 'discover/movie?api_key=', APIKEY, '&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=1&with_genres=', genre, '&without_genres=', withoutGenre, '&with_watch_monetization_types=flatrate');
    }
    fetch(url)
        .then(result => result.json())
        .then((data) => {
